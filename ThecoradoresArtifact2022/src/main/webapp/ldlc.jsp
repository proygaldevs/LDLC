...
<script type="text/javascript" src="js/fabric.js?versionCssJs=1"></script>
<!-- convierte en imagen el canvas -->
<script type="text/javascript" src="js/ldlctoimage.js?versionCssJs=1"></script>
<script type="text/javascript" src="js/jquery.js?versionCssJs=1"></script>
<script type="text/javascript" src="js/bootstrap.js?versionCssJs=1"></script>
<!-- para la herramienta quitar fondo -->
<script type="text/javascript" src="js/dat.js?versionCssJs=1"></script>
<script type="text/javascript" src="js/slic-segmentation.js?versionCssJs=1"></script>
<script type="text/javascript" src="js/underscore-min.js?versionCssJs=1"></script>
<script type="text/javascript" src="js/jsfeat-min.js?versionCssJs=1"></script>
<!-- para la subida manual de items -->
...
<script language="JavaScript" type="text/javascript">
		...
		// VARIABLES JAVASCRIPT GLOBALES PARA USAR EN GETiTEMS Y OTROS
		idDecorador=<%=id_decorador%>; 
		var urlItems='<%=new Config().URL_items%>';
		// NO SE USA
        function itemsExtension(){
        	console.log(tags);
        	getItems(0, 1, idDecorador, 0, '');
		}

        var id_proyecto="<%=id_proyecto%>";  
        var id_decorador="<%=id_decorador%>"; 
		// FUNCIONES PARA RECIBIR RESPUESTA DE LOS ENVIOS POR FORMS SIN REFRESCAR 
...
/* 			    console.log("alto y ancho de activeobject");
				...
				if(imagen_sin_fondo.tipo=="-3") { 
					activeObject.set("src", pathcompleto); 

					// AÑADIMOS ESTE NUEVO PASO A REHACER, DESACER
					...
					canvas.getActiveObject().set({width:W});
   					var json = JSON.stringify(canvas.toDatalessJSON(['title','tags','price','url','id','bloqueo','imagenOriginal'])); 
					stateJson++;lastJSONState[stateJson]=json; 
					document.getElementById('json').value = json;
					canvas.renderAll();
				...
				if(tags=="") {
					// para no cortar la carga de imagenes
					carga="on";
					getItems(0, 1, idDecorador, 0, '');
				}
			   }
			...
			{            
				...
				obj.unshift({"id":id,"precio": objetoParaInsertar.precio,"url": url, "path": pathcompleto,"titulo": objetoParaInsertar.titulo,"etiquetas": objetoParaInsertar.etiquetas,"imagenOriginal": pathcompleto });

				active_page=1;
				max_iconos=5;
				//$('#items-div').html('');decotheco
				mostrarItemsPagina('items-div',obj, active_page);
			    genPager('pager-container','items-div',obj);
			    //genPager('pager-container-borradores','items-div-borradores',obj);
			    mostrarFlechasTodos(); --%>
 
				// CUANDO INSERTAMOS UN ITEM MANUALMENTE O DESDE LA EXTENSIÓN
				if(tags=="") {
					getItems(0, 1, idDecorador, 0, '');
				}
				setTimeout(function(){ $('.modal').modal('hide');  }, 1000);
			...
			function repaintItems(){ 
				...
				%>
					obj.push({"id":<%=item.itemLDC_id%>,"precio": <%=item.Price%>,"url": '<%=item.URLImage%>', "path": '<%=new Config().URL_items+ item.PathImage%>',"titulo": '<%=item.ImageTitle%>',"etiquetas": '<%=etiquetas_obj.toLowerCase()%>',"imagenOriginal": '<%=new Config().URL_items+ item.PathImage%>'});
...
<!-- CUANDO LA PÁGINA ESTÁ CARGADA TRAEMOS ITEMS Y COMPROBAMOS SI EL DECORADOR SE CORRESPONDE CON LA LISTA Y PROYECTO -->
<body onload=" getItems(0, 1, idDecorador, 0, '');login('ldlc')" style="min-width: 1050px">
	...
	<div id="elcontainerfluid"
		...
		<!-- métodos manipulacion del canvas / get items... -->
		<script type="text/javascript" src="js/controller.js?versionCssJs=1"></script>
		...
		<div id="container">
		    ...
		    <img id="screen" src="">
		    <canvas width="1000" height="1007" id="screenCanvas"></canvas>
		...
		<script>
				...
				function handleDrop(e) {
				    ...
				    newImage = new fabric.Image(img, {
		                ...
		                imagenOriginal: img.getAttribute('src'),
				        left: pointer.x,
				        top: pointer.y,
	                ...
	                if (busqueda){
						if(tag){
							...
							getItems(page, pages, 0, donde, etiquetasFinal);  
	                	}else {
							console.log(pages);
							console.log(page);
							console.log(donde);
							console.log(etiquetasFinal);
							getItems(page, pages, idDecorador, donde, etiquetasFinal);  
	                	}
	                } else{
						if(tag){
							...
							success : function(data) {
								...
								if (isError(data)) {  
									...
									for(var i=0;i<data.length;i++){
										 ...
										 objSinFondo.push({"id":data[i].id, "precio":data[i].price, "url":data[i].url, "path": path+''+data[i].src,"titulo": data[i].title, "etiquetas": data[i].tags,  "imagenOriginal": path+''+data[i].src});
									 	
									}   
									 
									url_image = activeObject.get("src");
									imagenOriginal = activeObject.get("imagenOriginal");
									 
									active_page=1;
									max_iconos=5;
									if(objSinFondo.length==0){
										$("#items-div-sinfondo").css({"padding-top":"0", "display":"block"});
										if(url_image.indexOf("base64") > -1) {
					  						$("#items-div-sinfondo").html('<div class="image_wrapper2"><div class="image_background2" style="text-align: center;line-height: 89px;background-image: none!important;height:90px;position:relative" onclick="resetQuitarFondo();"><img class="clipper_image2 recargarQF" style="padding-left:0"  src="'+url_image+'"><img style="position:absolute;top:2px;left:0px;height:40px;margin-top: -8px" src="img/recargar.svg"></div></div>');
					  					
				  						$("#items-div-sinfondo").append('<div class="image_wrapper2" style="margin-right: 35px;"><div class="image_background2" style="text-align: center;line-height: 89px;background-image: none!important;height:90px;position:relative" onclick="resetOriginal();"><img class="clipper_image2 recargarQF" style="padding-left:0" src="'+imagenOriginal+'"><img style="position:absolute;top:2px;left:0px;height:40px;margin-top: -8px;" src="img/recargar.svg"></div></div>');
					  					} else {
					  						$("#items-div-sinfondo").html('<div class="image_wrapper2" style="margin-right: 35px;"><div class="image_background2" style="text-align: center;line-height: 89px;background-image: none!important;height:90px;position:relative" onclick="resetQuitarFondo();"><img class="clipper_image2 recargarQF" style="padding-left:0"  src="'+url_image+'"><img style="position:absolute;top:2px;left:0;height:40px;margin-top: -8px" src="img/recargar.svg"></div></div>');
		...
		<div class="container controls">
			...
			<ul class="nav nav-tabs  tab">
				<li id="miselementos" class="active"><a data-toggle="tab"
					onclick="totalItemsPaginado=0;donde=0;page=0;tags='';getItems(0, 1, idDecorador, donde, tags);"
			...
			<div class="tab-content">
				<div id="menumiselementos" class="tab-pane fade in active">
					...
					<div class="cargaSVGDecorador"><img src="/img/cargando-puntos.svg"></div>
					<div id="cuadroitems" class="cuadroitems">
						<div id="cuadroitemshead" class="cuadroitemshead">
							...
							<div id="lupa-container" class="lupa-container"
								onclick="active_page=1;donde=0;div_focus='items-div'; busqueda=true;tag=false;pager_container='pager-container';input_id='buscarinput'; showItemsByTagTodos(<%=id_decorador%>,'items-div','pager-container',document.getElementById('buscarinput').value); ">
						...
						<div id="items-container" class="items-container">
							...
							<script>		
										...
										for (z=0;z<=array_tags.length-1; z++){
											...
											for (x = 0; x <= obj.length; x++){
												...
												if (obj[x]!=null && obj[x]['etiquetas'].includes(array_tags[z])){
													...
													bandera=false;
													genDivItem(container_items,obj[x]['id'],obj[x]['path'],obj[x]['url'], obj[x]['precio'],0,obj[x]['path']);
								...
								function showItemsByTagTodos(id_decorador,container_items,container_pager,tag){
									if(tag==""){
										...
										for (i=0;i<array_tags.length; i++){ 
											if(array_tags[i].charAt(array_tags[i].length-1)=="s"){ 
												content.tipos.push(array_tags[i].substring(0,array_tags[i].length-1));
											} else { 
												content.tipos.push(array_tags[i]);
											}
										}
										// la primera página de la búsqueda será 
										var content = JSON.stringify(content);  
										tags=content;

										pagina=0;
										page=0;
										totalItemsPaginado=0;
										pages=1;
										max= calcularMaxItems();
										items=max;
										getItems(0, 1, idDecorador, donde, content); 
								      		...
								      		$.ajax({
								      			...
								      			success : function(data) {
								      				...
								      				if (isError(data)) {  
	  													...
	  													for(var i=0;i<data.length;i++){ 
	 														 path='<%=new Config().URL_items%>';
								      						 obj2.push({"id":data[i].id, "precio":data[i].price,"url":data[i].url, "path": path+''+data[i].src,"titulo": data[i].title, "etiquetas": data[i].tags, "imagenOriginal": path+''+data[i].src, "tipo": data[i].tipo});
								      				...
								      				if (isError(data)) {  
	  													...
	  													for(var i=0;i<data.length;i++){
	 														 ...
	 														 console.log(data[i].tipo);
								      						 obj.push({"id":data[i].id, "precio":data[i].price, "url":data[i].url, "path": path+''+data[i].src,"titulo": data[i].title, "etiquetas": data[i].tags, "imagenOriginal": path+''+data[i].src, "tipo": data[i].tipo});
				...
				<div id="menutodos" class="tab-pane fade">
					<div class="cargaSVG"><img src="/img/cargando-puntos.svg"></div>
					<div id="cuadroitemstodos" class="cuadroitems">
						<div id="cuadroitemstodoshead" class="cuadroitemshead">
							...
							<div id="lupatodos-container" class="lupa-container"
								onclick="active_page=1;div_focus='items-div2';busqueda=true;tag=true;donde=1;pager_container='pager-container-todos';input_id='buscartodosinput';showItemsByTagTodos(0,'items-div2','pager-container-todos',document.getElementById('buscartodosinput').value);">
								<img width="20" height="20" src="img/buscar.png">
							</div>
							<div id="lupatodos-container" class="lupa-container"
								onclick="active_page=1;div_focus='items-div2';busqueda=true;tag=true;donde=1;pager_container='pager-container-todos';input_id='buscartodosinput';showItemsByTagTodos(0,'items-div2','pager-container-todos','')">
								<img width="20" height="20" src="img/refresh.png">
							</div>
										...
										<div class="icons-container">
											...
											<div id="icono-mobiliario<%=i%>"
												...
												<img alt="<%=etiqueta%>"
													...
													onclick="active_page=1;div_focus='items-div2';busqueda=true;tag=true;donde=1;pager_container='pager-container-todos';showItemsByTagTodos(0,'items-div2','pager-container-todos','<%=etiqueta%>');document.getElementById('buscartodosinput').value='<%=etiqueta%>';" />
											</div>
										...
										<div class="icons-container">
											...
											<div id="icono-complementos<%=j%>"
												...
												<img alt="<%=etiqueta%>"
													...
													onclick="active_page=1;div_focus='items-div2'; busqueda=true;donde=1;tag=true;pager_container='pager-container-todos';showItemsByTagTodos(0,'items-div2','pager-container-todos','<%=etiqueta%>');document.getElementById('buscartodosinput').value='<%=etiqueta%>';" />
											</div>
											<% 	
														j=j+1;
													}
								...
								<div id="extras-container" class="mobiliario-icons">
										...
										<div class="icons-container">
											...
											<div id="icono-extras<%=k%>"
												...
												<img alt="<%=etiqueta%>"
													...
													onclick="active_page=1;div_focus='items-div2';donde=1; busqueda=true;tag=true;pager_container='pager-container-todos';showItemsByTagTodos(0,'items-div2','pager-container-todos','<%=etiqueta%>');document.getElementById('buscartodosinput').value='<%=etiqueta%>';" />
											</div>
											<% 	
														k=k+1;
													}
									...
									<script>
												$('#buscarinput').keypress(function (e) {
												  if (e.which == 13) {
													  active_page=1;div_focus='items-div'; busqueda=true;tag=false;pager_container='pager-container';donde=0;input_id='buscarinput'; showItemsByTagTodos(<%=id_decorador%>,'items-div','pager-container',document.getElementById('buscarinput').value);
												    return false;
												  }
												});
												$('#buscartodosinput').keypress(function (e) {
													  if (e.which == 13) {
														  active_page=1;div_focus='items-div2';busqueda=true;tag=true;pager_container='pager-container-todos';donde=1;input_id='buscartodosinput';showItemsByTagTodos(0,'items-div2','pager-container-todos',document.getElementById('buscartodosinput').value);
													    return false;
													  }
													});
												</script>
 ...
 */
		...
		<div class="modal-dialog ">
			...
			<div class="modal-content">
				<div class="modal-body" style="width: 92%; padding: 0"
					...
					<a class="boton" data-toggle="modal" onclick="borradores();getBorradores('<%=new Config().decoradores_url %>',<%=id_decorador%>);cerramosONo=false;" data-target="#modal-borradores">BORRADORES</a> 
						<a class="boton" data-toggle="modal" onclick="borradores();getPublicados('<%=new Config().decoradores_url %>',<%=id_decorador%>);cerramosONo=false;" data-target="#modal-publicados">PUBLICADOS</a>
				</div>
			</div>
		</div>
	</div>
	...
	// carga el nombre, etiquetas y tipo de estancia ()
	var habitacion="";
	var nombreLdlc=""; 
	<%if (lc.Canvas!="{}"){%>
