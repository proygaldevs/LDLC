...
function createLinks(){ 
	$("#vrToggle, #vrToggle .icon").click(function() { 
		...
		var href = pathname + 'vr.html?vr='+idVr+'&idvr='+idProyect;
...
function iniciarTodo(data) {
  ...
  var scenes = data.scenes.map(function(data) {
    ...
    data.infoHotspots.forEach(function(hotspot) {
	  object2 = new Object();
	  var bandera=true;
	  for(var i=0;i<enlacesHotspot.length;i++){
		  if(enlacesHotspot[i].title==hotspot.title) {
			  bandera=false;
		  }
	  }
	  if(enlacesHotspot.length==0) {
		  bandera=true;
	  }	  
	  ...
	  if(bandera==true) {
		  object2.title = hotspot.title;
		  object2.texto = hotspot.text; 
		  enlacesHotspot.push(object2);
		  enlacesHotspot2.push(object2);
	  }
	  } else {
		  object2.title = hotspot.title;
		  object2.texto = hotspot.text; 
	...
	function crearEnlaces() {
		for(var i=0;i<enlacesHotspot.length;i++) { 
			...
			var linkText = document.createTextNode(enlacesHotspot[i].texto);
			a.classList.add("scene");
			var li = document.createElement('li');
			li.appendChild(linkText); 
			li.classList.add("text");
			li.addEventListener("click", pulsar);  
			var id=enlacesHotspot[i].title;
			li.id = id;
			a.appendChild(li); 
			document.getElementsByClassName('scenes')[0].appendChild(a); 
			// <a class="scene" data-id="1-comedor"><li class="text">comedor</li></a>
		}
	} 
  ...
  function createLinkHotspotElement(hotspot) {
	    ...
	    icon.src = 'img/icono-siguiente-vista.svg';
  ...
  function createInfoHotspotElement(hotspot) {
	    ...
	    // Create image element.
	    var icon = document.createElement('img');
	    icon.src = 'img/icono-siguiente-estancia.svg';
	    icon.classList.add('link-hotspot-icon');
		...
		wrapper.addEventListener("click", pulsar);  
		var id=hotspot.title;
		wrapper.id = id; 
...
try {
	...
	house=getParameterByName('house');
	if(parseInt(idProyect)>1){
...
$("#vrBottom").click(function() {
	var idVr=$(".scenes .current").attr("data-id"); 
	//var href = pathname + 'tiles/'+idVr+'/vr';
	var href = urlbase + '/marzipano-master%202/demos/device-orientation/';
	window.parent.location.href = href;
});

function pulsar() {
	var id=this.id; 
	window.parent.insertar(id); 
};
