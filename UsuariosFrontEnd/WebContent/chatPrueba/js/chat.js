...
function logoutFunction() {
	...
	localStorage.removeItem('id_proyectoglobal'); 
	location.reload();
}
...
function checkCookiepaso1() {   
	...
	loginP = localStorage.getItem('loginP');
...
function loginForPAso1(lg_user, lg_pass) {
	...
	id_proyectoglobal = localStorage.getItem('id_proyectoglobal'); 
		...
		$.ajax({ 
					...
					success : function(data) {
						...
						if (id_ajax < 0) {
							...
							getNotificaciones(id_ajax, 1, id_proyectoglobal);
							...
							html3+='	<div style="float:left;border-bottom:0;margin-left:0;width:60%;padding-left: 15px;padding-top: 15px"><div style="float:left;height: 50px;border:1px solid #8c8686; max-width: 50px;border-radius: 50%;overflow:hidden"><img src="'+urlbuckets3 + 'decoradores/'+proyectoObject.uniqueDecorador+'/perfiles/cara/'+proyectoObject.caraDecorador+'" style="float:left;height:100%;"></div><p class="titulo" style="margin-bottom:0px">'+proyectoObject.nombreDecorador+'</p><p class="tituloProyecto"><a href="'+urlbase+'/paso1.html?id='+proyectoObject.id+'" style="color:black;font-weight:normal">'+proyectoObject.tituloProyecto+'</a></p></div><div id="listaProyectos" class="navbar-toggle" data-toggle="modal" data-target="#proyectosLista"><span class="sr-only">Toggle navigation</span><span class="icon-bar"></span><span class="icon-bar"></span><span class="icon-bar"></span></div>'; 
...
function cambioChat(user_ajax, id_del_proyecto_a_cargar) { 
	...
	localStorage.setItem('id_proyectoglobal', JSON.stringify(id_del_proyecto_a_cargar)); 
	//location.reload(); 
