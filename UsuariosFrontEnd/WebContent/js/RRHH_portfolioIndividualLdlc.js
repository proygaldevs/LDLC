//Vars for links.js
var clientKey='ttm78dfyvwav';


var client = stream.connect(clientKey, null, '41688');
var user1 = client.feed('User', '84', '{{ user_feed_1.token }}');
	...
	html += '<div style="list-style:none;padding-left:15px;padding-right:15px" class="filtros">';
		...
		$.each(arrayItems, function(i, item) {
			...
			urlProducto = processUrlAffiliates(item[0].url, listaAfiliados, idDecorador);
				...
				if(item[0].price>0 ){
					html+='<div class="col-xs-12 col-sm-6 col-md-4 col-lg-3" ><div class="cuadros2"><div class="imagenItems"><img style="'+style+'" src="'+item[0].src+'" alt="item'+item.length+'"/> </div><div class="letra-xs letra-mayusculas nombreItem" style="margin-bottom:5%;width:90%;margin-left:5%;border-bottom:1px solid #ccc; text-overflow:ellipsis; white-space:nowrap; overflow:hidden;letter-spacing: 2pt!important ">'+item[0].title+'</div><a class="buttonstandard_invertido letra-xs" style="width:70%;margin-bottom:5%;padding:0;letter-spacing:1px" target="_blank" href="'+urlProducto+'">VER +</a></div></div>';
