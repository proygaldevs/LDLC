...
function insertaselementos(){ 
	if(proyectoObject.projectsTypes==undefined || proyectoObject.projectsTypes.precio!="179") {
		var href = urlbase ; 
		window.location = href;
	} else {
		...
		if(orden=="360") { 
			if(proyectoObject.data360!=undefined) {
				...
				if(tipo==4) {
					var promotora=proyectoObject.piso[0].promotora;
					console.log(proyectoObject); 
					html+='<div class="col-xs-12">';
					html += '<div class="center-block" style="width:150px;height:150px;border-radius:50%;overflow:hidden;margin-bottom: 20px;"><img style="text-align:center;width:auto;height:100%" src="'+urlbuckets3+'promotoras/'+proyectoObject.piso[0].idPiso+'.jpg" alt="'+promotora+'"></div>';
					html += '<a href="'+proyectoObject.piso[0].url+'" class="letra-m" style="color:black;text-transform:uppercase">'+promotora+'</a>';
					html += '</div>';
					html+='</div>';
				}
			} else {
			...
			if(tipo=="4"){  
				...
				html +='<div class="col-xs-12" style="margin-bottom:50px"><div class="col-xs-12 col-xs-offset-0 col-md-6 col-md-offset-3"><img usemap="#mapa1" style="width:100%;position:relative" src="'+urlbuckets3+'pisos/'+idPiso+'.jpg"></div></div>';
				html +='<map name="mapa1">';
				
				for(j=0;j<proyectoObject.piso.length;j++) {
					html +='<area idproyect="'+proyectoObject.piso[j].idProject+'" class="d'+proyectoObject.piso[j].idProject+'" style="cursor:pointer" href="#" alt="'+proyectoObject.piso[j].nombre+'" title="'+proyectoObject.piso[j].nombre+'" coords="'+proyectoObject.piso[j].coordenadas+'" shape="poly">';
				}
				html +='</map>'; 
			}
		...
		if(tipo!=4) {
				...
				html+='<a class="buttonstandard  "onclick="nuevoProyecto('+proyectoObject.idDecorador+')">contratar</a></div>'; 
					...
					html+='<a class="buttonstandard" href="contacto-contract.html">contactar</a></div>'; 
				}
			...
			if(l>1) {
				...
				if(proyectoObject.data360!=undefined) { 
					...
					if(proyectoObject.propuestas[1].imagenes.length>=2) {
						...
						if(proyectoObject.propuestas[1].imagenes.length>=2) {
							html += '<div id="myCarousel5" class="myCarousel5 carousel slide col-xs-12" data-ride="carousel"  data-interval="true" style="padding-top:40px;">';
				...
				if(tipo=="4"){
					...
					.mapster({
						...
						mapKey: 'idproyect', fillColor: "A2AAA8", staticState: true, selected:false,stroke: true, strokeColor: 'ff0000', strokeOpacity: 1,strokeWidth: 1,

					    onClick: function() {
					        idProyecto=this.getAttribute('class');
					        $('area').mapster('deselect');
					        $('.'+idProyecto).mapster('select'); 
					        id=idProyecto.substr(1);
					        ChangeUrl('Page1', urlbase+"/piso-decoracion-online.html?id="+id); 
					...
					$('.d'+proyectoObject.id).mapster('select');
					$('.d'+proyectoObject.id).mapster({staticState: true, stroke: true, strokeColor: 'bc8b07', strokeOpacity: 1,strokeWidth: 1, });
				}
				...
				$(window).resize(function(){
					...
					if(tipo=="4"){
						...
						var id_del_proyecto_a_cargar = getParameterByName("id");  
						$('.d'+id_del_proyecto_a_cargar).mapster('select');
					}
				}); 
	}		
