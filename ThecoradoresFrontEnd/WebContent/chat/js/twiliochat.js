...
function recibeNotification(mensaje){
	 
	try {
		var nombreProyecto=notificacion[0];
		var id_project=notificacion[1];
		var nombreEnvia=notificacion[2];
		var id_user=notificacion[3];
		var id_decorador=notificacion[4];
		var fromto=0;
		var type=0;
		var other=mensaje;
...
function cambioEstadoChat(message) {
    function cargarAlertaAuto(message) {
    	message=messageChat(message);
		if(message!=""){ 
			var probando=tc.currentChannel.sendMessage(message); 
	    	probando.then(function(channel) { 
		    	var ultimoMensaje=$(".message-body").last().html(); 
		    	if(ultimoMensaje!=mensaje) {
			        tc.currentChannel.getMessages(1)
    ...
    members.then(function(currentMembers) {
        ...
        currentMembers.forEach(function(member) {   
        	if(member.identity == nombreToChat){  
	        	...
	        	contador++;
        		var usuario=member.getUser(); 
        		usuario.then(function(user) {
        				if(user.state.online==null || user.state.online=="null" || user.state.online==false){}
        				else {
        					estado=user.state.online;
        				}
        	        	if(currentMembers.length==contador) { 
        	        		cargarAlertaAuto(message); }
        ...
        currentMembers.forEach(function(member) {  
        	if(member.identity == nombreToChat){  
        		...
        		usuario.then(function(user) {   
    	        	...
    	        	contador++;
    	        	if(currentMembers.length==contador) { cargarAlert(); }
        		})       	
        	} 
        });
        function cargarAlert() { 
        	if(estado==false) {  
            	tc.currentChannel.getMessages(1)
                .then(function(messages) {
              	  //console.log(messages); 

                if(messages.items[0].state.author==nombreToChat) {
                    var mensaje=messages.items[0].state.body;
...
function updateMemberMessageReadStatus(){
	...
	cerrarCargando();
	//alert("la carga inicial ha acabado (Aquí podriamos quitar el loader del chat");
...
function lleganMensajes(message){
	if(flagCargaInicialAcabada==false){
		...
		if(message.author == nombreToChat || message.author === nombreAnt){
			//alert("es mio");
...
var notificacion=[];
var nombreAnt="";
function goChat(nombre, canal, historial, array) {
  
	tc = {};
	notificacion=array;
	nombreAnt=nombre;
	nombreUpdate=nombre+"_"+canal;
	nombreToChat=nombre+"_"+canal;
  ...
  function handleInputTextKeypress(event) {
	  ...
	  if (event.keyCode === 13) {
		  	mensaje=$(this).val();
	        $(this).val('');
	    	mensaje=mensaje.replace(new RegExp('\\n','g'),'');
 
		    if(mensaje!="" && mensaje!="\n" && mensaje!=" ") {
	    	 
				//updateMemberMessageReadStatus(); 
		    	var probando=current.sendMessage(mensaje); 
		    	probando.then(function(channel) { 
			    	var ultimoMensaje=$(".message-body").last().html(); 
			    	if(ultimoMensaje!=mensaje) {
				        tc.currentChannel.getMessages(1)
  ...
  function initChannelEvents() {
    ...
    tc.currentChannel.on('memberUpdated', function(member) {  
        if(member.identity==nombreToChat){
        	comprobarOnline()
        } 
    });
  ...
  tc.addMessageToList = function(message) {
	    ...
	    if (message.author === tc.username || message.author === nombreAnt) {
	      rowDiv.children("div").addClass('own-message');
	    } else {
		      rowDiv.children("div").addClass('other-message');
	    }
	    rowDiv.addClass('anchoMsg');
	  ...
	  function addMessageToListWellcome () {
		    ...
		    rowDiv.loadTemplate($('#message-template'), {
		      ...
		      body: "Todavía no hay ningún mensaje con el usuario... , escríbele cuando quieras para cualquier consulta, dale la bienvenida, infórmale... :)"
		    });
